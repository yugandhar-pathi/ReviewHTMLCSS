it("Dispaly student details", () => {
    ...
    cy.get("[id=deleteWarningDlg]").should("be.visible")
    cy.get("[id=removeStd]").click()
    cy.get("[id=studentUlId]").children().should("have.length", 5)

    //Display student information
    cy.get("[id=student2]").click()
    cy.get("[id=displayStudentDetailsDlg]").should("be.visible").contains("Leia")
    cy.get("[id=closeDialogStudentDetails]").click()
    cy.get("[id=displayStudentDetailsDlg]").should("not.be.visible")

    //Add student
    cy.get("[id=addStudent]").click()
    cy.get("[id=addStudentDlg]").should("be.visible")
